...
type gameTable2V13 struct {
	...
	GameGenres []*gameGenreTableV13 `gorm:"many2many:game_genre_relations;joinForeignKey:GameID;joinReferences:GenreID"`
}

//nolint:unused
func (*gameTable2V13) TableName() string {
	return "games"
}

type gameGenreTableV13 struct {
	ID        uuid.UUID `gorm:"type:varchar(36);not null;primaryKey"`
	Name      string    `gorm:"type:varchar(32);not null;unique"`
	CreatedAt time.Time `gorm:"type:datetime;not null;default:CURRENT_TIMESTAMP"`
	// 後方参照を使っているためポインタになっている。
	// 参考: https://gorm.io/ja_JP/docs/many_to_many.html#%E5%BE%8C%E6%96%B9%E5%8F%82%E7%85%A7%EF%BC%88Back-Reference%EF%BC%89
	Games []*gameTable2V13 `gorm:"many2many:game_genre_relations;joinForeignKey:GenreID;joinReferences:GameID"`
}

//nolint:unused
func (*gameGenreTableV13) TableName() string {
	return "game_genres"
}
...
func v13() *gormigrate.Migration {
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gorm.DB) error {
			err := tx.Exec("ALTER TABLE games ADD COLUMN latest_version_updated_at DATETIME NOT NULL DEFAULT created_at").Error
			if err != nil {
				return err
			}

			err = tx.AutoMigrate(&gameTable2V13{})
			if err != nil {
				return err
			}
			err = tx.AutoMigrate(&gameGenreTableV13{})
			if err != nil {
				return err
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			err := tx.Migrator().DropColumn(&gameTable2V13{}, "latest_version_updated_at")
			if err != nil {
				return err
			}
			err = tx.AutoMigrate(&gameTable2V12{}, &gameGenreTableV12{})
			if err != nil {
				return err
			}
			return nil
