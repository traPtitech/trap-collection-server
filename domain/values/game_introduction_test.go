...
func TestNewGameIntoductionIDFromString(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		_, err := NewGameIntoductionIDFromString(test.id)
	...
	err := quick.Check(func(id id) bool {
		_, err := NewGameIntoductionIDFromString(uuid.UUID(id).String())
		return err == nil
	}, nil)
	if err != nil {
